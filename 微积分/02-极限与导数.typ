...
对于 $A + B$ 形式的等价无穷小替换, 如果替换了之后的项没有互相消去的话, 是可以用的.
...
- $1^infinity$ 型极限: $(1 + Delta)^infinity ~ e^(infinity Delta)$ 即转换成求指数上的极限.
- $lim_(x -> 0^+) x^alpha ln^beta x = 0 quad (alpha > 0, beta > 0)$
- 极限表示为两个函数之差, 可以用拉格朗日定理换差为导, 如 $lim_(x -> infinity) x^2 [arctan(x+1) - arctan x]$
...
- 区间 $(A, B)$ 上的弦: $y = (f(A) - f(B)) / (A - B) (x - B) + f(B)$
- 区间内任一点 $P(x_0, y_0)$ 的切线: $y = f'(x_0) (x - x_0) + f(x_0)$

当函数是凹的时候 (即 $f''(x) > 0$) 时, 区间内恒有: 

$
  y_"弦" (x) > f(x) > y_"切" (x)
$

凸区间亦有类似结论. 这种条件有时候可以拿来做放缩.

], [
  #image("../assets/凹凸下的弦与切.png")
])
