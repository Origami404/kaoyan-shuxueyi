...
- $m$ 阶 B 树每个节点有 $ceil(m / 2)$ 到 $m$ 个子树, 然后有对应数量减一的键 (即 $floor(m / 2)$ 到 $m - 1$ 个键), 子树分布在键之间, 包含两个键之间的值
- 主要操作是分裂与合并
...
- 生成归并段: 将外部文件切分成若干份, 使每份有序
  - 朴素做法: 切分后加载进内存做内部排序
  - 置换-选择: 
    - 用一个有限大小工作区当成窗口, 滑过全部数据
    - 记录每次放出去的数据, 在下一次选择刚刚好比上一次放出的数据大的、窗口内满足条件的最小的数. 这是我们在有限的视野能选出的最适合的能放进这个段里的数.
    - 如果当前窗口中没有比上一次放出的数据大的, 就新开一个段
- 合并归并段
  - 朴素做法
    - 开 $m$ 个缓冲区, 然后每次遍历每个缓冲队列的头, 取最小的元素放进输出缓冲队列中
    - 输入队列空了就做磁盘 IO; 输出队列满了就做磁盘 IO
  - 败者树
    - 每个缓冲区顶部元素当成叶节点, 两两比较, 小的那个贡上去继续比, 大的留在非叶子节点里
    - 每次更新只需要从被取走元素的叶子节点开始, 每层向上和被留在非叶子节点的大的元素比较, 直到把整个树更新掉
...
- 线程
  - 内核级线程就是指内核管理和实现的线程, 不是给内核用的线程
  - 用户级线程就是普通程序自己实现和调度的线程
...
- 介质控制
  ...
  - 半双工线路防止争用: CSMA/CD
    ...
    - $"最短帧长" = "单向传播时延" tau times "链路速率" times 2$
- VLAN
...
- TCP 基础
  - TCP 的序号是数据的字节序号 (不算头长), 不是 MSS 序号
    - *区分包的序号和 ACK 包的确认序号*, 确认序号为 $x$ 的包的 ACK 包的确认序号是 $x colored(+ 1)$
